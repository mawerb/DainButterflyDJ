...
function setupHttpServer(config, tools, services, toolboxes, metadata, privateKey, contexts, pinnables, datasources = []) {
    ...
    app.use("*", async (c, next) => {
        // Skip signature for ping endpoint
    ...
    app.use("*", async (c, next) => {
        ...
        // Skip signature check for ping endpoint
        if (c.req.path.startsWith("/ping")) {
            await next();
            return;
        }
    ...
    app.get("/oauth2/providers", async (c) => {
        ...
        const agentInfo = await getAgentInfo(c);
        const providers = await app.oauth2.getProviders(agentInfo.id);
    ...
    app.get("/datasources", (c) => {
        const datasourceInfo = datasources.map((datasource) => ({
            id: datasource.id,
        ...
        return c.json(datasourceInfo);
    });
    app.post("/datasources/:datasourceId", async (c) => {
        const datasource = datasources.find((datasource) => datasource.id === c.req.param("datasourceId"));
        if (datasource) {
            const agentInfo = await getAgentInfo(c);
    ...
    app.get("/getAllDatasources", async (c) => {
        const agentInfo = await getAgentInfo(c);
